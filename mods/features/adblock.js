...
function directFilterArray(arr, page, context = '') {
  ...
  // ⭐ Check if this is a playlist page
  const isPlaylistFilterPage = (page === 'playlist');
  ...
  if (isPlaylistFilterPage && DEBUG_ENABLED) {
  ...
  if (isPlaylistFilterPage && window._lastHelperVideos.length > 0 && arr.length > 0) {
  ...
  if (DEBUG_ENABLED && (shouldFilterShorts || shouldHideWatched)) {
    ...
    console.log('[FILTER_START #' + callId + '] Is Playlist:', isPlaylistFilterPage);
  ...
  if (isPlaylistFilterPage && filtered.length === 0 && arr.length > 0) {
  ...
  if (isPlaylistFilterPage && filtered.length > 0 && noProgressBarCount > 0) {
