...
- **Do not set the TV's Developer Mode "Host PC IP" to `127.0.0.1`/localhost.** That value makes standalone use its CDP/SDB-based injection path, which was extensively tested and found unreliable on both Tizen 5.5 and 6.5 (works once, then fails on every subsequent launch until the app's cache is cleared — see `AGENTS.md` "Known unresolved issues" for the full investigation). Leave Host PC IP set to anything else (e.g. a real PC's IP, or unset) and standalone uses its local-proxy path instead, which has been reliable on both TVs in testing. If you also use TizenBrew or another PC-based debugging tool (like Samsung's own [WITs](https://github.com/Samsung/Wits)) that needs Host PC IP set to your actual PC's address, that's compatible with standalone's proxy path — it's only the `127.0.0.1` value specifically that causes problems here.
